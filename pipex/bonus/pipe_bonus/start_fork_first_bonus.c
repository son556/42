...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   start_fork_first_bonus.c                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: chanson <chanson@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/16 16:00:29 by chanson           #+#    #+#             */
/*   Updated: 2023/01/20 14:38:37 by chanson          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	start_fork_first(t_cmd *cmd, char **envp, int idx)
{
	...
	if (pid == 0)
	{
		...
		if (dup2(cmd->infile, STDIN_FILENO) == -1)
			str_put_error("dup2 error child first", cmd);
		if (dup2(cmd->pipe_fd[idx][WRITE], STDOUT_FILENO) == -1)
			str_put_error("dup2 error child first fd2", cmd);
		if (execve(cmd->cmd_addr, cmd->argvv, envp) == -1)
			str_put_error("cmd option error child first", cmd);
	}
	else
	{
